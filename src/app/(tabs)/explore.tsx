...
function WarehouseItemCard({ item, canManageAll, symbolsLoaded, onEdit, onExtract, onDelete }: { item: WarehouseItem; canManageAll: boolean; symbolsLoaded: boolean; onEdit: () => void; onExtract: () => void; onDelete: () => void }) {
  ...
  const [movements, setMovements] = useState<WarehouseItemMovement[]>([]);
  ...
  useEffect(() => {
    ...
    void getWarehouseItemMovements(item.id)
      .then((nextMovements) => { if (mounted) setMovements(nextMovements); })
      .catch((error) => { if (mounted) setMovementError(error instanceof Error ? error.message : 'No se pudieron cargar los movimientos.'); })
      .finally(() => { if (mounted) setIsLoadingMovements(false); });
    return () => { mounted = false; };
  }, [isExpanded, item.id, item.updatedAt]);

  return <ThemedView type="backgroundElement" style={[styles.itemCard, item.status === 'extracted' && styles.extractedCard]}>
    <Pressable accessibilityRole="button" accessibilityState={{ expanded: isExpanded }} onPress={() => setIsExpanded((expanded) => !expanded)} style={styles.itemMainRow}>
    ...
    <View style={styles.itemBody}>
      ...
      <View style={styles.cardFooter}><View style={styles.actionStack}><ThemedText style={[styles.status, item.status === 'extracted' && styles.extractedStatus]}>{item.status === 'extracted' ? `Extraído · ${formatDate(item.extractedAt)}` : 'Disponible'}</ThemedText><View style={styles.actions}>{canChange ? <Pressable accessibilityLabel="Extraer" onPress={(event) => { event.stopPropagation(); onExtract(); }} style={styles.actionButton}><ThemedText style={styles.materialIcon}>{symbolsLoaded ? 'remove_circle_outline' : '−'}</ThemedText></Pressable> : null}<Pressable accessibilityLabel="Editar" onPress={(event) => { event.stopPropagation(); onEdit(); }} style={styles.actionButton}><ThemedText style={styles.materialIcon}>{symbolsLoaded ? 'edit' : '✎'}</ThemedText></Pressable>{canManageAll ? <Pressable accessibilityLabel="Eliminar" onPress={(event) => { event.stopPropagation(); onDelete(); }} style={styles.actionButton}><ThemedText style={styles.materialIcon}>{symbolsLoaded ? 'delete' : '×'}</ThemedText></Pressable> : null}</View></View></View>
    </View>
    </Pressable>
    {isExpanded ? <View style={styles.movementHistory}>
      <View style={styles.movementHeader}><ThemedText themeColor="textSecondary" style={styles.movementHeaderText}>Fecha</ThemedText><ThemedText themeColor="textSecondary" style={styles.movementHeaderText}>Cantidad</ThemedText><ThemedText themeColor="textSecondary" style={styles.movementHeaderText}>Movimiento</ThemedText></View>
      {isLoadingMovements ? <ActivityIndicator size="small" /> : null}
      {movementError ? <ThemedText style={styles.movementError}>{movementError}</ThemedText> : null}
      {!isLoadingMovements && !movementError && !movements.length ? <ThemedText themeColor="textSecondary" style={styles.emptyMovements}>Sin movimientos registrados.</ThemedText> : null}
      {movements.map((movement) => <View key={movement.id} style={styles.movementRow}><ThemedText style={styles.movementDate}>{formatMovementDate(movement.createdAt)}</ThemedText><ThemedText style={styles.movementQuantity}>{formatQuantity(movement.quantity)} {formatUnit(item.unitType)}</ThemedText><View style={[styles.movementChip, movement.type === 'entry' ? styles.entryChip : styles.exitChip]}><ThemedText style={[styles.movementChipText, movement.type === 'entry' ? styles.entryChipText : styles.exitChipText]}>{movement.type === 'entry' ? 'Entrada' : 'Salida'}</ThemedText></View></View>)}
    </View> : null}
  </ThemedView>;
}
...
function formatMovementDate(date: string) { return new Intl.DateTimeFormat('es-CU', { day: '2-digit', month: 'short', year: '2-digit' }).format(new Date(date)); }
...
const styles = StyleSheet.create({
  ...
  itemCard: { alignItems: 'stretch', backgroundColor: '#FFFFFF', borderColor: '#EAE6DF', borderRadius: Spacing.three, borderWidth: StyleSheet.hairlineWidth, minHeight: 128, overflow: 'hidden', padding: Spacing.two }, itemMainRow: { alignItems: 'stretch', flexDirection: 'row', width: '100%' }, extractedCard: { opacity: 0.62 }, itemImage: { borderRadius: Spacing.two, height: 112, width: 112 }, imageFallback: { alignItems: 'center', backgroundColor: '#EEF1F3', borderRadius: Spacing.two, height: 112, justifyContent: 'center', width: 112 }, imageFallbackText: { color: '#4D96FF', fontSize: 26, fontWeight: '800' }, itemBody: { alignSelf: 'stretch', flex: 1, justifyContent: 'space-between', paddingLeft: Spacing.three }, itemName: { fontSize: 16, fontWeight: '800' }, itemMeta: { fontSize: 12, marginTop: 2 }, dateRow: { flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.one, marginTop: 3 }, itemDate: { fontSize: 10 }, cardFooter: { alignItems: 'center', flexDirection: 'row', justifyContent: 'flex-end' }, actionStack: { alignItems: 'center', backgroundColor: '#FFFCF5', borderRadius: Spacing.two, flexDirection: 'row', gap: Spacing.one, paddingHorizontal: Spacing.one, paddingVertical: 2 }, status: { color: '#258D42', fontSize: 10, fontWeight: '800' }, extractedStatus: { color: '#8A5D00' }, actions: { flexDirection: 'row', gap: 2 }, actionButton: { alignItems: 'center', height: 26, justifyContent: 'center', width: 26 }, movementHistory: { borderTopColor: '#EAE6DF', borderTopWidth: StyleSheet.hairlineWidth, gap: Spacing.one, marginTop: Spacing.two, paddingHorizontal: Spacing.one, paddingTop: Spacing.two, width: '100%' }, movementHeader: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, movementHeaderText: { flex: 1, fontSize: 10, fontWeight: '700' }, movementRow: { alignItems: 'center', flexDirection: 'row', minHeight: 28 }, movementDate: { flex: 1, fontSize: 11 }, movementQuantity: { flex: 1, fontSize: 11, fontWeight: '700' }, movementChip: { borderRadius: 999, minWidth: 58, paddingHorizontal: Spacing.one, paddingVertical: 3 }, movementChipText: { fontSize: 10, fontWeight: '800', textAlign: 'center' }, entryChip: { backgroundColor: '#DDF5E8' }, entryChipText: { color: '#258D42' }, exitChip: { backgroundColor: '#FDE2E2' }, exitChipText: { color: '#C24141' }, emptyMovements: { fontSize: 11, paddingVertical: Spacing.one, textAlign: 'center' }, movementError: { color: '#C24141', fontSize: 11, textAlign: 'center' },
